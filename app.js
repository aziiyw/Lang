...
const data = {
  ...
  vocab: [
    { word: '非常口', reading: 'hijōguchi', meaning: 'Emergency exit', category: 'Directions', seen: 1, level: 15, status: 'New', place: 'Kyoto Station' },
...
const sampleTranslation = { text: 'Emergency exit', native: '非常口', romanisation: 'hijōguchi', natural: 'You’ll see this on signs at stations, shops, and hotels — it points to an emergency exit.' };

let state = { screen: 'welcome', mode: 'learn', showQuiz: false, quizAnswered: false, query: '', category: 'All', uploadedImage: '', analysis: null };
...
function result() {
 const isLearn = state.mode === 'learn';
 const scan = state.analysis || { translation: sampleTranslation.text, detectedText: sampleTranslation.native, romanisation: sampleTranslation.romanisation, naturalNote: sampleTranslation.natural, vocabulary: [{ word:'非常口', reading:'hijōguchi', meaning:'emergency exit', category:'Directions', importance:'high', box:{x:315,y:390,width:370,height:140} }] };
 ...
 return shell(`<section class="page result-page"><div class="result-top"><button class="back" data-action="scan">←</button><div class="mode-chip">${isLearn ? '✦ Learning from your find' : '◌ Quick translation'}</div><button class="more">•••</button></div>
  <div class="source-preview ${state.uploadedImage ? 'uploaded-preview' : ''}" ${state.uploadedImage ? `style="background-image:linear-gradient(#173b3d88,#173b3d88),url('${state.uploadedImage}')"` : ''}>${state.uploadedImage ? `<span class="upload-label">Your scan</span>${scan.vocabulary?.slice(0,3).map(v=>`<i class="live-highlight" style="left:${v.box.x/10}%;top:${v.box.y/10}%;width:${v.box.width/10}%;height:${v.box.height/10}%"><b>${v.word}</b></i>`).join('')}` : `<div class="sign-sample"><span>KYOTO STATION</span><b>非常口</b><small>EMERGENCY EXIT</small></div><div class="word-highlight">非常口</div>`}</div>
...
function learningPanel(word) { return `<div class="learning-panel"><div class="section-heading"><div><p class="eyebrow">TAKE A LITTLE WITH YOU</p><h2>Useful words here</h2></div><span class="count">1 worth keeping</span></div>
 ...
 <div class="first-met"><span>⌖</span><p><b>First met at Kyoto Station</b><small>March 28 · your first day in Kyoto</small></p></div>
...
function render() { const views = { welcome, setup, modeChoice, home: dashboard, scan, result, loading, vocab, trip, quiz }; views[state.screen](); }
...
function uploadTravelImage(input, mode) {
  ...
  reader.onload = async () => {
    try {
      const apiResponse = await fetch('/api/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image: reader.result }) });
      if (apiResponse.ok) state.analysis = await apiResponse.json();
    } catch (_) {
      // The local static preview has no server; it intentionally falls back to the Kyoto demo.
    }
    state.screen = 'result';
    render();
  };
