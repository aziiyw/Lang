...
function uploadTravelImage(input, mode) {
  ...
  reader.onload = async () => {
    try {
      ...
      if (!apiResponse.ok) throw new Error((await apiResponse.json()).error || 'The translation service is unavailable.');
      state.analysis = await apiResponse.json();
